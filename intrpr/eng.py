...
class Intrpr:
    def __init__(
        ...
        self.env_vars.set("_USR_DIR_", pl.Path("~").expanduser())
    ...
    def execute(self, ln: str) -> int:
        ...
        for tok_grp, sp_chr in self.parser.parse(ln):
            ...
            if cmd_src == "external":
                ...
                if pid == 0:
                    ...
                    # because the OS only recognises 8-bit ints for exit codes
                    os.close(w)
                    packed = os.read(r, 4)
                    ret_code = st.unpack("i", packed)[0]
